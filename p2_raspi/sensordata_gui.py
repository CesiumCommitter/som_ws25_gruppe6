...
def save_csv():
    ...
    # 1. Open the 'Save As' dialog to choose where CSV file will be saved
    ...
    if file_path:
        try:
            # open selected file in write mode and create CSV writer
            with open(file_path, mode='w', newline='') as file:
                writer = csv.writer(file)

                # 3. Write the Header Row of CSV file
                writer.writerow(["Timestamp", "Temperature (C)", "Humidity (%)"])
                # Write all collected data rows (timestamp, temperature, humidity)
                for row in zip(xar, yar, yar2):
                    writer.writerow(row)
            # Print success message after saving the file
            print(f"Data successfully saved to {file_path}")
        # Handle errors that may occur during file saving
        except Exception as e:
            print(f"Error saving file: {e}")


# Create a frame at the bottom of the window to hold control buttons
